# Local Emailing

Capture local-business email strategy, personalization rules, and lightweight
follow-up structure.

## Module summary

- **Surface:** `Playbook`
- **Audience:** outbound operator, GTM lead, growth engineer
- **Primary action:** read the playbook
- **Status:** first release

## Start here

- [`PLAYBOOK.md`](./PLAYBOOK.md)
- [`EXAMPLES.md`](./EXAMPLES.md)

## Focus

- short first-touch emails
- highly personalized subject lines
- lead-with-value offers
- follow-up cadence
- when email-first beats call-first

## What this module covers

- how to write `2-3` sentence SMB outbound emails
- what personalization is worth using
- when to offer help before asking for a meeting
- how to structure a `3-4` touch cadence
- concrete email shapes based on local-business selling patterns

## Source material

- internal operating guidance from Openmart
- patterns seen from Openmart customers using local-business data in outbound
- examples inspired by teams like NewsBreak and POS providers using Openmart data
